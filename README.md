...
The right-hand status item shows **`identity.mvs`** from the configured manifest (truncated when long). If the manifest is missing, the label shows **MVS —** and a warning color; click it to **initialize** the repo (`mvs-manager init`) or open MVS settings. When the manifest is valid, click for a quick pick: open manifest, lint, generate, doctor, or output.
...
- **MVS: Initialize manifest (init)** — `mvs-manager init` (same as the status bar “no manifest” path, with confirmation).
